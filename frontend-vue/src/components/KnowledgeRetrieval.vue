<template>
    ...
    <div v-if="logs.length" class="retrieval-logs-list">
      <div class="retrieval-log-card" v-for="log in logs">
        ...
        <div class="retrieval-log-card-body">
          ...
          <div>
            <el-button @click="view(log)">查看详情</el-button>
            <el-button type="danger" @click="deleteLog(log.id)">删除</el-button>
          </div>
        </div>
      </div>
    </div>
  ...
  <el-dialog v-model="dialogVisible" title="检索详情">
    <div class="retrieval-detail-container">
      ...
      <div>
        <h3 class="retrieval-detail-title">检索到的知识项 ({{ logInfo.retrievedItems?.length }})</h3>
        <div class="retrieval-detail-block">
        </div>
      </div>
    </div>
  ...
  if (res.ok) {
    ...
    logs.value = data.logs.map((log: RetrievalLog) => {
      ...
      if (typeof retrievedItems == 'string') {
        try {
          retrievedItems = JSON.parse(retrievedItems);
        } catch (error) {
          retrievedItems = [];
        }
      ...
      let hasResult = false, itemCount = 0;
      retrievedItems.forEach((id: string) => {
        if (id !== '_has_results') {
          itemCount++;
        } else {
          hasResult = true;
      ...
      log.timeDiff = -dayjs(log.createdAt).diff(new Date(), 'hour');
